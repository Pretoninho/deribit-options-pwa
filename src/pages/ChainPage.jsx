...
function calcDIRate(ivPct, days) {
  ...
  const T = days / 365
  return (ivPct / 100 * Math.sqrt(T) * 0.4 * 100) * (365 / days)
}
...
export default function ChainPage() {
  ...
  const [diView, setDiView] = useState('strike') // 'strike' | 'multi'
  ...
  // Charger données multi-échéances (ATM IV + taux DI par expiry)
  const loadMulti = async () => {
    ...
    for (const ts of expiries) {
      try {
        ...
        const marketRate = calcDIRate(iv, days)
        const minRate    = calcMinRate(iv, days)
  ...
  const diRows = rows.map(r => {
    ...
    return { strike: r.strike, iv, marketRate, minRate, distPct,
      isBuyLow: distPct != null && distPct < 0,
      isSellHigh: distPct != null && distPct > 0 }
  })
  ...
  // Smile: IV par strike pour l'échéance sélectionnée dans chaîne
                  ...
                  return (
                    <div key={r.strike} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:5 }}>
                      ...
                      <div style={{ flex:1, height:6, background:'rgba(255,255,255,.05)', borderRadius:3, overflow:'hidden' }}>
                        <div style={{ height:'100%', width:`${barPct}%`, background: color, borderRadius:3, transition:'width .3s' }} />
                      </div>
                      <span style={{ width:44, fontSize:10, color: isPeak?'var(--atm)':color, fontWeight: isPeak?700:400, textAlign:'right', flexShrink:0 }}>
                        {r.iv.toFixed(1)}%
                        {isPeak && <span style={{ fontSize:8, marginLeft:2 }}>▲</span>}
                      </span>
                    </div>
                  )
                })}
                <div style={{ marginTop:8, fontSize:10, color:'var(--text-muted)', display:'flex', gap:16 }}>
                  <span><span style={{ color:'var(--call)' }}>■</span> OTM Put (Buy Low)</span>
                  <span><span style={{ color:'var(--atm)' }}>■</span> ATM</span>
                  <span><span style={{ color:'var(--accent2)' }}>■</span> OTM Call (Sell High)</span>
                </div>
              </div>
            </div>
          )}
            ...
            <>
              ...
              {diExpiry && atmIV && (
                <div className="card" style={{ marginBottom:12, borderColor:'rgba(255,215,0,.3)', background:'rgba(255,215,0,.03)' }}>
                  <div className="card-header" style={{ color:'var(--atm)' }}>⚡ ATM — {fmtTs(diExpiry)} · {diDays}j</div>
                  <div style={{ padding:'14px 16px', display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10 }}>
                    ...
                    <div>
                      ...
                      <div className="stat-sub">{marketRateATM ? (marketRateATM/100*(diDays/365)*100).toFixed(3)+'%' : '—'} / {diDays}j</div>
                    </div>
                    <div>
                      <div className="stat-label">Min. Nexo</div>
                      <div className="stat-value orange">{minRateATM?.toFixed(2)}% /an</div>
                      <div className="stat-sub">{minRateATM ? (minRateATM/100*(diDays/365)*100).toFixed(3)+'%' : '—'} / {diDays}j</div>
                    </div>
                  </div>
                </div>
              )}
          ...
          {diView === 'multi' && (
            <div className="fade-in">
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                <span style={{ fontSize:11, color:'var(--text-muted)' }}>IV ATM + taux DI par échéance</span>
              ...
              {multiLoading && (
                <div className="card"><div style={{ padding:20, textAlign:'center', color:'var(--text-muted)', fontSize:12 }}>Chargement de toutes les échéances…</div></div>
              )}
              ...
              {multiData.length > 0 && (
                <div className="card">
                      ...
                      <div key={r.ts} style={{
                        ...
                        <div>
                          <div style={{ fontFamily:'var(--sans)', fontWeight: isPeak?800:600, fontSize:12, color: isPeak?'var(--atm)':'var(--text)' }}>
                            {fmtTs(r.ts)}
                            {isPeak && <span style={{ fontSize:9, marginLeft:4, color:'var(--atm)' }}>🔥</span>}
                          </div>
                          <div style={{ fontSize:9, color:'var(--text-muted)', marginTop:1 }}>
                            ATM {r.atmStrike?.toLocaleString()}
                          </div>
                        </div>
                        <div style={{ textAlign:'center', fontSize:11, color:'var(--text-muted)', alignSelf:'center' }}>{r.days}</div>
                  ...
                  <div style={{ padding:'10px 14px', fontSize:10, color:'var(--text-muted)', lineHeight:1.7 }}>
                    🔥 = échéance avec IV la plus élevée · Marché = taux théorique · Min Nexo = seuil à 80%
                  </div>
                </div>
              )}

              {!multiLoading && multiData.length === 0 && (
                <div className="empty-state"><div className="empty-icon">◇</div><h3>Appuyez sur Charger</h3><p>Récupère l'IV ATM pour chaque échéance</p></div>
              )}
            </div>
          )}
        </div>
